...
| `FILESYSTEM_ROOT_PATH` | Filesystem MCP server root | `C:/Users/<you>/.GitHub/Programming-lead-AI-System-` | `/home/<you>/Programming-lead-AI-System-` |

If `MEMPALACE_PALACE_PATH` is unset, the MemPalace MCP server will fail to start — cross-run memory then falls back to `decision-log.md` + `memory.md` + `reports/*.md` per CLAUDE.md §7. If `FILESYSTEM_ROOT_PATH` is unset, the filesystem MCP server will fail to start. Heartbeat still runs; MCP status is reported per tick.
