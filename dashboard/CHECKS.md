...
One npm script per check. Each runs standalone, exits non-zero on failure, and is composable into the `check:all` pipeline (`node scripts/cohesion-check.js`, with `check:all:shell` as fallback).
...
| `check:all` | `node scripts/cohesion-check.js` | Runs all of the above in order with unified output/reporting, failing fast at first non-zero; `check:all:shell` remains the sequential `&&` fallback |
...
## Next
- **§A.2 (#23)** delivered `scripts/cohesion-check.js` as the `check:all` runner — better per-check output capture + JSON report.
