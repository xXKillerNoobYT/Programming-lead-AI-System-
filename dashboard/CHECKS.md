...
One npm script per check. Each runs standalone, exits non-zero on failure, and is composed by the `check:all` cohesion runner (`node scripts/cohesion-check.js`).
...
| `check:all` | `node scripts/cohesion-check.js` | Runs all checks in order with per-check output + JSON report (`check:all:shell` keeps the legacy `&&` chain) |
...
## Next
- **§A.2 (#23)** delivered `scripts/cohesion-check.js`; keep `check:all:shell` only as a compatibility fallback.
