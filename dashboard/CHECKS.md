...
One npm script per check. Each runs standalone, exits non-zero on failure, and is orchestrated by the `check:all` cohesion runner (`scripts/cohesion-check.js`).
...
| `check:all` | `node scripts/cohesion-check.js` | Single entry-point runner: executes all checks, captures output/duration/exit code, writes JSON report |
| `check:all:shell` | sequential `&&` chain | Legacy shell-chain fallback (no JSON report) |
...
## Next
- **§A.2 (#23)** delivered `scripts/cohesion-check.js` as the active `check:all` runner.
