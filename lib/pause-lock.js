 ...
 * @param {string}  [args.reason]    — human-readable reason string
 * @param {number}  [args.durationMs] — if set, pausedUntil = now + durationMs
 * @returns {{ path: string|null, pausedUntil?: string }}
 *
 * Contract:
 *   - `path` is always populated on success.
 *   - `pausedUntil` is populated iff `durationMs` was passed.
