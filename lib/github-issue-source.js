...
function parseIssueNumberFromUrl(url) {
  if (typeof url !== 'string') throw sourceError('parent issue URL is invalid');
  let parsed;
  try {
    parsed = new URL(url);
  } catch {
    throw sourceError('parent issue URL is invalid');
  }
  const match = parsed.pathname.match(/\/issues\/(\d+)$/);
  if (!match || !positiveInteger(Number(match[1]))) {
    throw sourceError('parent issue URL is not a GitHub issue URL');
  }
...
function normalizeIssue(raw) {
  ...
  return {
    ...
    parentNumber: raw.parent_issue_url === null || raw.parent_issue_url === undefined
      ? null
      : parseIssueNumberFromUrl(raw.parent_issue_url),
    childNumbers: [],
    blockedByNumbers: [],
  };
}
...
function dependencyNumbers(repository, issueNumber, expectedTotal, options) {
  ...
  const numbers = entries.map((entry) => entry && entry.number);
  if (numbers.some((number) => !positiveInteger(number)) || new Set(numbers).size !== numbers.length) {
