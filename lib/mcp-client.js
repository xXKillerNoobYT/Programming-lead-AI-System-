...
function loadMcpConfig(configPath) {
    ...
    try {
        ...
        for (const [name, serverConfig] of Object.entries(parsed.mcpServers || {})) {
            try {
                mcpServers[name] = expandEnvPlaceholders(serverConfig, `mcpServers.${name}`);
            } catch (err) {
                const base = (serverConfig && typeof serverConfig === 'object') ? serverConfig : {};
                mcpServers[name] = {
                    ...base,
                    __configError: err.message || String(err),
                };
            }
        }
        return { mcpServers };
...
async function connectServers(config) {
    ...
    const records = await Promise.all(
        entries.map(async ([name, serverConfig]) => {
            if (serverConfig && serverConfig.__configError) {
                return [name, { status: 'failed', error: serverConfig.__configError }];
            }
...
async function safeCallTool(record, toolName, args = {}) {
    ...
    if (!Array.isArray(record.tools)) {
        return { error: 'tool metadata unavailable' };
    }
...
function expandEnvPlaceholders(value, path) {
    if (typeof value === 'string') {
        return value.replace(/\$\{([A-Za-z_][A-Za-z0-9_]*)\}/g, (_match, varName) => {
            if (!(varName in process.env)) {
                throw new Error(`missing required env var "${varName}" at ${path}`);
            }
            return process.env[varName];
        });
    }
    if (Array.isArray(value)) {
        return value.map((item, idx) => expandEnvPlaceholders(item, `${path}[${idx}]`));
    }
