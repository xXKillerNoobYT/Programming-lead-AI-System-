...
/**
 * Expand ${VAR_NAME} placeholders against process.env. Returns either
 * `{ ok: true, value }` or `{ ok: false, missing: [vars...] }`.
 */
function expandEnvPlaceholders(value, envSource = process.env) {
    if (typeof value !== 'string') return { ok: true, value };

    const missing = new Set();
    const expanded = value.replace(ENV_PLACEHOLDER_RE, (_, varName) => {
        if (!Object.prototype.hasOwnProperty.call(envSource, varName) || envSource[varName] === undefined) {
            missing.add(varName);
            return '';
        }
    ...
    if (missing.size > 0) {
        return { ok: false, missing: Array.from(missing).sort() };
    }
    return { ok: true, value: expanded };
}

/**
 * Expand env placeholders in stdio fields (`command`, `args`, `env`).
 */
function expandStdioConfig(serverConfig, envSource = process.env) {
    const missing = new Set();
    const markMissing = (vars) => vars.forEach((v) => missing.add(v));

    const commandExpanded = expandEnvPlaceholders(serverConfig.command, envSource);
    if (!commandExpanded.ok) markMissing(commandExpanded.missing);

    const argsExpanded = (serverConfig.args || []).map((arg) => {
        const out = expandEnvPlaceholders(arg, envSource);
        if (!out.ok) {
    ...
    for (const [key, value] of Object.entries(serverConfig.env || {})) {
        const out = expandEnvPlaceholders(value, envSource);
        if (!out.ok) {
            markMissing(out.missing);
            continue;
        }
        envExpanded[key] = out.value;
    }
...
async function connectStdioServer(name, serverConfig) {
    const expanded = expandStdioConfig(serverConfig);
    if (!expanded.ok) {
        return {
            status: 'failed',
            error: `missing env var(s): ${expanded.missing.join(', ')}`,
        };
    }

    const transport = new StdioClientTransport({
        command: expanded.value.command,
        args: expanded.value.args,
        env: { ...process.env, ...expanded.value.env },
