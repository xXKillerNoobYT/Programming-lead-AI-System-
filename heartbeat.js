 ...
 *   node heartbeat.js --delegate-first-step
 *   node heartbeat.js --watch      # loop, default 60_000 ms between ticks
 *   HEARTBEAT_ENABLE_FIRST_DELEGATION_STEP=1 node heartbeat.js
...
const { resolveRepoPaths } = require('./lib/env-paths.js');
...
const {
    ...
    pauseLockPath: HEARTBEAT_PAUSE_LOCK_PATH,
} = resolveRepoPaths(REPO_ROOT);
...
function formatTickReport(state) {
    ...
    const delegationLine = delegation
        ? `- ${delegation.status}${delegation.detail ? ` — ${delegation.detail}` : ''}`
        : '- off';
...
function readGitState({ runShellImpl = runShell } = {}) {
    const branch = runShellImpl('git', ['branch', '--show-current']);
    const sha = runShellImpl('git', ['rev-parse', '--short', 'HEAD']);
    return parseGitState(branch, sha);
}

function readIssueCounts({ runShellImpl = runShell } = {}) {
    const json = runShellImpl('gh', ['issue', 'list', '--state', 'open', '--limit', '100', '--json', 'number,state,labels']);
...
function parseIssueListForDelegation(ghJson) {
    let issues;
    try {
        issues = JSON.parse(ghJson);
    } catch {
    ...
    return issues.filter((x) => x && Number.isInteger(x.number));
}

function runFirstDelegationStep({ runShellImpl = runShell, timestamp = new Date().toISOString() } = {}) {
    const json = runShellImpl('gh', [
        'issue',
        'list',
        '--state',
        'open',
    ...
    const issues = parseIssueListForDelegation(json);
    if (!issues.length) {
        return { status: 'skipped', detail: 'no in-progress issue found for delegation' };
    }
    const issue = issues[0];
    const body = `[heartbeat] first delegation step (${timestamp}): sync requested from assignee with AC/proof update.`;
    const out = runShellImpl('gh', ['issue', 'comment', String(issue.number), '--body', body]);
    if (!out.trim()) {
        return { status: 'failed', detail: `gh issue comment failed for #${issue.number}` };
    }
    return { status: 'posted', detail: `comment added to #${issue.number}` };
}

function readDelegationState(statePath) {
    try {
...
function writeDelegationState(statePath, state) {
...
function parseHeartbeatOptions(argv = [], env = process.env) {
    return {
        enableFirstDelegationStep:
            argv.includes('--delegate-first-step') ||
            envFlagEnabled(env.HEARTBEAT_ENABLE_FIRST_DELEGATION_STEP),
    };
}

function runFirstDelegationStepWithCooldown({
    runShellImpl = runShell,
    timestamp = new Date().toISOString(),
    statePath = join(REPORTS_DIR, 'delegation-state.json'),
    cooldownMs = Number(process.env.HEARTBEAT_DELEGATION_COOLDOWN_MS) || 6 * 60 * 60 * 1000,
} = {}) {
    const json = runShellImpl('gh', [
        'issue',
        'list',
    ...
    const issueNumber = String(issues[0].number);

    const state = readDelegationState(statePath);
    const now = Date.parse(timestamp);
    const last = state[issueNumber] && Date.parse(state[issueNumber]);
    if (Number.isFinite(now) && Number.isFinite(last) && now - last < cooldownMs) {
        return {
            status: 'skipped',
            detail: `cooldown active for #${issueNumber}`,
        };
    }
    const body = `[heartbeat] first delegation step (${timestamp}): sync requested from assignee with AC/proof update.`;
    const out = runShellImpl('gh', ['issue', 'comment', issueNumber, '--body', body]);
    if (!out.trim()) {
        return { status: 'failed', detail: `gh issue comment failed for #${issueNumber}` };
    }
    state[issueNumber] = timestamp;
    writeDelegationState(statePath, state);
    return { status: 'posted', detail: `comment added to #${issueNumber}` };
}
...
async function tick(clientsByName = {}, options = {}) {
    ...
    const pauseLock = readPauseLock(REPO_ROOT, { lockfilePath: HEARTBEAT_PAUSE_LOCK_PATH });
    if (pauseLock.paused) {
    ...
    const shell = options.runShellImpl || runShell;
    const timestamp = new Date().toISOString();
    const git = readGitState({ runShellImpl: shell });
    const issues = readIssueCounts({ runShellImpl: shell });
    ...
    const delegation = options.enableFirstDelegationStep
        ? runFirstDelegationStepWithCooldown({
            runShellImpl: options.runShellImpl,
            timestamp,
            statePath: options.delegationStatePath,
            cooldownMs: options.delegationCooldownMs,
        })
        : null;
...
async function main(argv = process.argv.slice(2)) {
    ...
    const tickOptions = parseHeartbeatOptions(argv, process.env);
    const intervalMs = Number(process.env.HEARTBEAT_INTERVAL_MS) || DEFAULT_INTERVAL_MS;

    console.log(`[heartbeat] connecting MCP servers from ${MCP_CONFIG} …`);
    ...
    try {
        await tick(clients, tickOptions);
    } catch (err) {
    ...
    const timer = setInterval(async () => {
        try { await tick(clients, tickOptions); }
