...
- **Branching strategy — git-flow-lite** (per **D-20260418-026**; user directive 2026-04-18). Permanent branches: (1) `main` — production-quality, always-green, user-approved-for-release; receives PRs only from `beta` (periodic sync) or rare `hotfix/*`. (2) `beta` — integration branch; expected to pass tests but may have edge-case bugs; receives PRs from `feature/*` and `bugfix/*`. Short-lived branches: `feature/issue-<N>-<slug>`, `bugfix/issue-<N>-<slug>`, `hotfix/issue-<N>-<slug>`, `meta/<slug>` (doc-only/branch-tag ticks per D-025). **Heartbeat default**: Station 4 (Branch) branches off `beta`; Station 11 (Merge) targets `beta`. `beta → main` sync happens on its own cadence — user-initiated or at phase boundaries. Stale feature/bugfix branches (no commit ≥ 7 days) should be closed (merged or abandoned) — fallback = merge into `beta`. `beta` branch is created off `main` once the current 15-PR stack-recovery (Q-20260418-002 / D-25 / Option A click-through) lands in `main`; until then this bullet is forward-pointing policy, not live state.
