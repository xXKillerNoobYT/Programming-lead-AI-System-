...
- **Self-pacing cadence** — per **D-20260418-009**, retuned faster by **D-20260418-014**, made mandatory by **D-20260418-032**, **default 60s per D-20260418-043** (user directive 2026-04-18: *"a minute after the last one that would be cool"*). **Every tick — no exceptions** — ends with `ScheduleWakeup(delaySeconds = clamp(ideal, 60, 270), prompt = "<<autonomous-loop-dynamic>>")`. **Default ideal = 60s** (the floor — ~1 minute re-entry). 270s reserved for: (a) Singular-Heartbeat collision detected (don't race a peer), (b) user explicitly asks to slow down. Live user + empty backlog no longer uses 270s — it uses 60s too; a live user can interrupt by typing. Range stays within the 5-min prompt-cache TTL either way.
