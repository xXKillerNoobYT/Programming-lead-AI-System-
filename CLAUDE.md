...
- **`status:needs-user` label — the "on you" queue** (per **D-20260418-033**; user directive 2026-04-18: *"on an issue and tag it user answer this queue please"*). Apply `status:needs-user` to any Issue that is blocked on the user — manual PR merge, design-question answer, approval for a hard-to-reverse action, or a go/no-go decision. These Issues appear as the user's visible queue (`gh issue list --label status:needs-user`). Heartbeat pick rules treat `status:needs-user` as **skip — not my turn**: `issue-triage-picker` (Station 2) excludes them from its ranking. When filing a Dev-Q&A question, also open a tracking Issue labeled `status:needs-user` and cross-link the Q-ID, so the question surfaces in the user's Issue list instead of requiring them to periodically check `Docs/Plans/Dev-Q&A.md`. Remove the label when the user acts (merges the PR, answers the question, approves the action) — the tick that captures the answer also strips the label before closing the Issue.
