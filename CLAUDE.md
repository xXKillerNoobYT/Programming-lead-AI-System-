...
- **Self-pacing cadence** — per **D-20260418-009**, retuned faster by **D-20260418-014**, made mandatory by **D-20260418-032** (user directive: *"ScheduleWakeup this needs to be part of the auto system that the coding agent uses"*). **Every tick — no exceptions** — ends with `ScheduleWakeup(delaySeconds = clamp(ideal, 60, 270), prompt = "<<autonomous-loop-dynamic>>")`. No "user is live → skip" out. A live user can interrupt the wake-up by typing; a sleeping session needs the schedule to survive idle gaps — so the safe default is **always fire**. Minimum 60s (ScheduleWakeup's hard floor), maximum 270s (≈4.5 min, stays under the 5 min prompt-cache TTL so re-entry is cache-warm and cheap). Singular-heartbeat rule (below) still applies — one tick must finish before the next starts.
