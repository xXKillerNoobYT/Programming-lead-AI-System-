...
- **Branch instruction-file sync — main is canonical, propagate to every branch** — per user directives 2026-04-19: *"the main branch should have the latest updates when it comes to those"* + *"keep your files up to date between branches that way even when you're switching to an old branch or something that you have all the information that you should have and that if your instructions got updated for some reason or another that you keep the updates"*. Every branch — new, old, resumed, or freshly-created — must carry the latest instruction files (`CLAUDE.md`, `SOUL.md`, `.claude/**`, `AI plans/**`, `architecture.md`, `memory.md`). Concretely:
  ...
  2. **On every checkout of an existing feature branch**, first `git fetch origin main`, then check whether any instruction file has diverged from `origin/main`. If any has, run `git merge origin/main` BEFORE doing feature work. This applies even when resuming old / dormant / superseded branches — the rule is about instruction parity, not commit recency
  ...
  4. **Supersession sweep as corrective action**: PRs with net-negative diffs vs main (more deletions than additions by a lot) are usually branches that ignored step 2 — close them as superseded (D-20260418-012) rather than rebasing
  5. **Detection helper (until a CI job exists)**: `git diff origin/main -- CLAUDE.md SOUL.md ".claude/**" "AI plans/**" architecture.md memory.md` — if non-empty on a feature branch with no instruction-file changes in its own commits, merge main first
