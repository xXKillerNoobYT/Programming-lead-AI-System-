...
- **Permission to fail, say "I don't know", ask questions** (per **D-20260418-038**; user directive 2026-04-18: *"you have permission to fail, you have permission to say I don't know, you have permission to ask questions"*). Standing permission — especially for auto-loop ticks. Don't manufacture certainty, don't fabricate confident answers to ambiguous inputs, don't commit busy-work to look productive. When uncertain: use `post-dev-qa` + `status:needs-user` Issue (D-033 pattern), or say "I don't know" in prose and escalate. Do not mistake "being helpful" for "always producing output."
- **Idle-tick cadence — A + D synthesis** (per **D-20260418-038** resolving Q-20260418-003): when an autonomous wakeup finds no pickable work (stack drain blocks new `feature/*` starts, no tracking Issues to close, no Dev-Q&A to answer), the tick writes a **terse** no-op record (one D-ID line + 10-line run-report) for audit-trail continuity AND pivots to `plans/*.md` decomposition work when the plan has a fuzzy area that will produce next-tick-pickable chunks. No pure-no-op is allowed to run more than 5 ticks in a row without re-asking via Dev-Q&A for fresh direction (cadence escape hatch).
- **`status:needs-user` label — the "on you" queue** (per **D-20260418-033**, refined by **D-20260418-039**; user directives 2026-04-18). Apply `status:needs-user` to any Issue blocked on the user — manual PR merge, design-question answer, approval for a hard-to-reverse action, or a go/no-go decision. These Issues appear as the user's visible queue (`gh issue list --label status:needs-user`). Heartbeat pick rules treat `status:needs-user` as **skip — not my turn**: `issue-triage-picker` (Station 2) excludes them from its ranking. **For Q-posting** (Dev-Q&A + Issue cross-link): post Q-block in `Docs/Plans/Dev-Q&A.md` AND open a GH Issue labeled BOTH `type:question` (new, purple) + `status:needs-user`. User answers in Issue comments (preferred) or the Dev-Q&A file. Follow-up questions go in the SAME Issue's comments — user welcomed iterative clarification. **Claude Code closes Q-Issues** (user explicitly opted out per *"I will not close the questions I'll leave that up to you"*); the tick that captures the answer transcribes to `decision-log.md`, removes the Q-block, strips `status:needs-user` (keeping `type:question` for audit), and closes with D-ID citation.
...
- **TDD is THE development method** (per **D-20260418-009** + **D-20260418-028**, user directive 2026-04-18: *"test-driven-development is how we want to make new parts of the program and updates"*). **All new program parts and updates go through red→green→refactor.** No production code without a failing test first. Applies equally on agent side (`/heartbeat` Station 5) and runtime side (`heartbeat.js` delegated-task contract). Exempt ONLY: docs (`*.md`, `*.txt`), config files (`.json`, `.yml`, `.toml`, `.npmrc`, `.env*`), generated files, and throwaway-script diagnostics that leave no artifact in the tree. Exempt changes must declare "TDD exempt — <category>" in the run report (one line). Tightened from the prior "pragmatic default" wording per user reinforcement; closes Issue #40.
...
- **Self-pacing cadence — three-tier adaptive** — per **D-20260418-009/014/032/043/151/153**. Every tick ends with `ScheduleWakeup(delaySeconds = clamp(ideal, 60, 3600), prompt = "<<autonomous-loop-dynamic>>")`. Three tiers:
  - **Tier 1 = 60s** (default, 0-2 consecutive no-ops, cache-warm)
  - **Tier 2 = 270s** (≥3 consecutive no-ops D-151, Singular-Heartbeat collision, or user-slowdown; still cache-warm)
  - **Tier 3 = 3600s** (≥6 consecutive no-ops D-153, or all-work-blocked-on-unanswered-Qs; crosses 5-min cache TTL — pays cache-miss on re-entry but cuts quota burn ~80% during long Q-blocked waits; user: *"like an hour or actively wait without LLM calls"*)
  - Detect no-op streak via `decision-log.md` backwards-scan. Any real-work tick resets the counter to 0.
- **Singular heartbeat — no concurrent peer** (per **D-20260418-013**; user directive 2026-04-18 "Option D"). Only ONE `/heartbeat` tick runs at a time. Spawning a second while one is running is not allowed. If a tick sees parallel-session signals mid-flight — D-ID collisions, Run-N title thrashing, unattributed commits on its branch, `decision-log.md` gaining entries it didn't write — **commit what it has, note the collision in the run report, and end the tick**. Do not race. This closes Issue #42. Applies equally to `/heartbeat` (Claude Code side) and `heartbeat.js` (runtime side; the runtime must take a session-lock before any tick). **Abort commit MUST go to `meta/abort-run-<N>` branch, NEVER direct-to-main (per D-20260418-061)** — three direct-to-main abort/summary commits on 2026-04-18 cascaded every MERGEABLE PR to CONFLICTING, burning Copilot quota; the meta-branch abort path avoids this.
- **Direct-to-main is forbidden except hotfixes** (per **D-20260418-061**; incident 2026-04-18). Aborts, Run summaries, and any other heartbeat-authored commits must go to a `meta/<slug>` branch or the current feature/bugfix branch and reach main via PR. Only `hotfix/issue-<N>-<slug>` branches may PR directly against `main`, and only with explicit user authorization per incident. Future direct-to-main commits by heartbeat sessions are D-061 violations — file Dev-Q&A for RCA when detected.
...
- **Branching strategy — git-flow-lite** (per **D-20260418-026** + tightened by **D-20260418-030**; user directives 2026-04-18). Permanent branches: (1) `main` — production-quality, always-green, user-approved-for-release; receives PRs only from `beta` (periodic sync) or rare `hotfix/*`. (2) `beta` — integration/stabilization branch; expected to pass tests but may have edge-case bugs; receives PRs from `feature/*` and `bugfix/*`. Short-lived branches: `feature/issue-<N>-<slug>`, `bugfix/issue-<N>-<slug>`, `hotfix/issue-<N>-<slug>`, `meta/<slug>` (doc-only/branch-tag ticks per D-025). **Heartbeat default**: Station 4 (Branch) branches off `beta`; Station 11 (Merge) targets `beta`. `beta → main` sync happens on its own cadence when a stable-enough version is ready — user-initiated or at phase boundaries.
  - **Concurrency cap (D-030)**: at most **ONE** active non-meta in-dev branch (`feature/*` OR `bugfix/*`) at a time. If one is in-flight and unmerged, the heartbeat CONTINUES it — does not spawn a second. Station 2 (Pick) checks for an open in-flight branch before selecting a new Issue. Exception: `hotfix/*` (user-authorized per incident) and `meta/<slug>` (doc-only) don't count toward the cap.
  - **3-day dev-time window (D-030)**: an active in-dev branch may sit up to 3 days as long as progress is being made OR it's not yet production-ready. Past 3 days of stagnation → close (merge to beta at minimum) OR file a Dev-Q&A explaining why still open. Distinct from the 6-hour supersession grace (D-027), which applies only to PRs whose commits are already in `main`.
  - **Lifecycle**: `feature/bugfix` → PR to `beta` → stabilization → `beta → main` sync when stable → next feature. One feature/goal to production before the next.
  - **Branch count: target 6, hard cap 15** — user directives 2026-04-18. **Hard cap = 15** (D-20260418-163/164) — remote branches on `origin` excluding `main`, counted via `git branch -r | grep -v 'HEAD\|main$' | wc -l`. When at or above 15: no new branches until drain. **Target = 6 total including main (D-20260418-166 + D-20260418-167)** — user: *"target of 5 branches: 1. main 2. beta 3. bugfix 4. new feature 5. other/any"* + *"maybe a 6. one for hardening the code."* Role-slot map: main + beta + 1 `bugfix/*` + 1 `feature/*` + 1 `harden/*` + 1 catchall (`hotfix/*` OR `meta/*` — one at a time). **Hardening branch (`harden/*`)** covers security patches, performance tuning, test-coverage lifts, refactors-for-maintainability — distinct from bugfix (fixing broken behavior) and feature (new capability). This **relaxes D-030's one-in-flight rule** across role-slots: 1 of each role may coexist (feature, bugfix, harden, catchall). Two branches of the same role-slot still violate D-030. Station 2 Pick checks the target and prefers continuing existing work; Station 4 Branch refuses creation if the role-slot is occupied.
  - **Branch-level completion (D-20260418-040)**: even after the primary `closes #N` Issue closes, before marking the branch done verify CI green + no unchecked `- [ ]` items in PR body + no TODOs in the diff that should be their own Issue (Polsia Rule 2 captures). *"Focus on a branch and get all the tasks completed on that branch so you can close it properly."* OK to pick a stale open branch and finish it rather than always starting fresh — stale-branch-completion counts as the one-in-flight branch per D-030. Close **properly**: merge (or close-with-reason) AND delete the branch. No orphan merged-but-not-deleted branches.
  - **Three completion gates (D-20260418-041/042)**: each branch must pass **completed** (scope + checklist + no dangling TODOs) + **tested** (CI green + TDD red→green evidence) + **merged** (squash + branch deleted) before "done." Skipping any gate = not done.
  - **Priority = closest-to-done, not oldest-first** (D-20260418-042, user directive 2026-04-18: *"prioritize them based on which one can be fully completed, tested, and merged first"*). When picking the next branch, survey all open branches + assess unchecked-task-count, orphan-status, CI state, blockers → pick the branch that can be fully drained soonest. Tiebreaker: fewer unchecked items, then has-closes-ref, then smaller diff.
  - **Q&A per branch with blockers** (D-20260418-042): when a branch has a design question or blocker, file a Q-posting (Dev-Q&A + `type:question`+`status:needs-user` Issue per D-039) AND reference the branch in the Q body. Branch with an open Q = skip + move to next-closest-to-done.
  - **Answered Q promotes the branch** (D-20260418-042, user directive: *"when I answer them you should address them promptly making that branch the next one in line to complete"*). The tick that transcribes the user's answer also immediately picks up the branch the Q was blocking (overrides closest-to-done ranking for that tick).
  - Stale feature/bugfix branches (no commit ≥ 7 days) must close — merge to `beta` or abandon.
  - `beta` is created off `main` once the current 15-PR stack-recovery (Q-20260418-002 / D-025 / Option A click-through) lands in `main`. Until then this bullet is forward-pointing policy; live state flips in the first post-drain tick.
- **PR supersession grace period — 6 hours** (per **D-20260418-027**, retuning **D-20260418-012**'s prior 3-day window per user directive 2026-04-18 *"forget about the three day wait thing … make it 6 hours"*). Heartbeat Station 11b.c — on first detection of a CONFLICTING+DIRTY PR whose commits are all in `main`, post a timestamped `"Superseded by ... Auto-close scheduled for <now + 6h> if no new activity"` comment; do NOT close this tick. On subsequent ticks, close only if (a) the comment exists, (b) the scheduled time has passed, (c) no new commits landed since. 6 hours matches the faster self-pacing cadence (D-014) and the live-development pace this session demonstrated; 3 days was set when the loop cadence was 15–60 min, not 1–4.5 min.
...
- **GitHub Copilot delegation** (per **D-20260418-045**): comment `@copilot <instruction>` on any Issue or PR to delegate small well-scoped work — merge conflict resolution, typos, import sorts, simple refactors, straightforward Issues. NOT for complex work or design-ambiguous tasks (those stay with me + Dev-Q&A). Invocation: `gh pr comment <N> -b "@copilot <instruction>"` or `gh issue comment <N> -b "@copilot <instruction>"`. Delegation does not relax D-028 (TDD red→green evidence still required on Copilot's code merges) or D-013 (Copilot is a tool, not a concurrent `/heartbeat` peer). See memory `reference_copilot_delegation.md` for full when/how/scope.
