...
- **Self-pacing cadence — adaptive** — per **D-20260418-009**, retuned faster by **D-20260418-014**, made mandatory by **D-20260418-032**, default 60s per **D-20260418-043**, now **adaptive per D-20260418-151** (user answered Q-005 with **D** 2026-04-18). **Every tick — no exceptions** — ends with `ScheduleWakeup(delaySeconds = clamp(ideal, 60, 270), prompt = "<<autonomous-loop-dynamic>>")`. **Ideal = 60s normally**, escalates to **270s** on ANY of: (a) Singular-Heartbeat collision, (b) user-slowdown-directive, (c) **≥3 consecutive no-op ticks** (reset to 60s on next real-work tick). Detect no-op streak by scanning `decision-log.md` backwards for the last non-no-op D-ID entry. Range stays within the 5-min prompt-cache TTL either way.
